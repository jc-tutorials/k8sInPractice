# Lab: configuration, state, and load — Podman

For laptops running **Podman** or Podman Desktop. On Docker Desktop, use `../docker` instead.

Three labs, roughly 20 minutes each, one per act. Everything here has been run end to
end, so if something doesn't behave the way it says, that's worth shouting about.
...
### 0a. One-time Podman setup

k3d talks to Podman through its Docker-compatible socket, and needs three things from
it. Each check below is quick; only run the fix if a check fails. On macOS:

**The Podman machine runs rootful.** Check with:

```bash
podman machine inspect --format '{{.Rootful}}'
```

If that prints `false`:

```bash
podman machine stop && podman machine set --rootful && podman machine start
```

**The Docker socket points at Podman.** Check with:

```bash
ls -l /var/run/docker.sock
```

It should point into a `podman` directory. If it doesn't exist, or points elsewhere:

```bash
sudo "$(command -v podman-mac-helper)" install && podman machine stop && podman machine start
```

**Containers can resolve `host-gateway`.** k3d needs this, and Podman's VM doesn't set
it by default. Check with:

```bash
podman run --rm --add-host probe:host-gateway docker.io/library/alpine:3.20 true
```

If that fails with `host containers internal IP address is empty`:

```bash
podman machine ssh -- "sudo mkdir -p /etc/containers/containers.conf.d && printf '[containers]\nhost_containers_internal_ip = \"192.168.127.254\"\n' | sudo tee /etc/containers/containers.conf.d/50-host-gateway.conf"
```

No restart needed. `192.168.127.254` is the address the Podman machine already uses
for your Mac; to undo it, delete that one file.
...
so autoscaling works. Nothing else to install. For the first minute, commands may print
warnings about `metrics.k8s.io`; they're harmless and stop once metrics-server is up.

### 0c. Build the app and hand it to the cluster

```bash
podman build -t localhost/exchange-gateway:v1 ./app
k3d image import localhost/exchange-gateway:v1 -c exchange
```

Podman names every image it builds `localhost/<name>`, so the `localhost/` prefix is
part of the image's real name — in the import and in every manifest. Leave it off and
Kubernetes looks for `docker.io/library/exchange-gateway:v1` instead, tries Docker Hub,
and the pods sit in `ImagePullBackOff`.
...
> `kind load docker-image localhost/exchange-gateway:v1 --name exchange`.
...
```bash
podman build -t localhost/exchange-gateway:v2 ./app
k3d image import localhost/exchange-gateway:v2 -c exchange
```
...
```bash
kubectl set image deployment/gateway gateway=localhost/exchange-gateway:v2
kubectl rollout status deployment/gateway
```
...
```bash
kubectl set image deployment/gateway gateway=localhost/exchange-gateway:v1
kubectl rollout status deployment/gateway
```
...
**`k3d cluster create` fails with `host containers internal IP address is empty`.**
Podman can't resolve `host-gateway`. Run the third fix in **0a**, then create the cluster
again. Upgrading k3d does not help.

**`k3d` says it can't connect to the Docker daemon.** The Docker socket isn't pointing at
Podman. Run the second fix in **0a**.
...
**`ImagePullBackOff`.** Check the image name has the `localhost/` prefix in both the import
and the manifest. Otherwise we skipped `k3d image import`, or we're pointed at the wrong
cluster. Check with `kubectl config current-context`; it should say `k3d-exchange`.
